...
import Ann14BubbleGif from '@/components/Events/GoldenWeek';
...
const prizeNameMap = { P1: '免费彩金', P3: '乐币', P4: '国庆大礼包', P5: '没有奖励' };
...
export default class ann14 extends Component {
	...
	needLogin = () => {
		if (!checkIsLogin()) {
			redirectToLogin('?from=event_nationalday2022');
			return true;
		}
		return false;
	};
	...
	showMyPrizes = async () => {
		...
		// Pushgtagdata('Engagement_Event', 'View', 'MyPrize_NationalDay2022​');
	...
	afterOpenPrizePopup = () => {
		...
		//增加 ?日期數字 用來強制加載動畫
		const imgUrl =
			`${process.env.BASE_PATH}` + '/img/events/Golden-Week/Pop-Up_BG.gif' + '?' + new Date().getTime();
	...
	drawPrize = async (clickBubble = true) => {
		...
		if (phase != 2) return;
		if (clickBubble) {
			// Pushgtagdata('Engagement_Event', 'Claim', 'Popping_NationalDay2022​');
		} else {
			// Pushgtagdata('Engagement_Event', 'Click', 'GrabMore_NationalDay2022​');
		}
	...
	clickDeposit = () => {
		// Pushgtagdata('Deposit_Nav', 'Click', 'Deposit_NationalDay2022​');
		Router.push('/deposit');
	};
			...
			>
				<div className="ann14-container">
					<div className="ann14-top">
						<ReactIMG className="ann14-top-banner" src="/img/events/Golden-Week/Text_CN.png" />
						<div className="ann14-top-text-container">
							<div className="ann14-top-text">点击烟花赢奖励</div>
						</div>
							...
							{this.state.showPrizeResult ? (
								<div className="ann14-prize-popup-content">
									...
									<ReactIMG
										...
										src={'/img/events/Golden-Week/Prizes/CN_' + this.state.prizeType + '.png'}
									/>
					...
					<div className="ann14-middle">
						...
						<div className="ann14-middle-content">
							...
							<div className="ann14-middle-button-container">
								<div
									...
									onClick={() => {
										// Pushgtagdata('Engagement_Event', 'View', 'TnC_NationalDay2022​');
						...
						>
							...
							<div className="ann14-middle-popup-content">
								1. 此活动开放给所有乐天堂会员<br />
								活动时间： 2022年10月1日 00:00:01 至2022年10月7日 23:59:59。 (北京时间）<br />
								<br />
								2. 参加方式:<br />
								...
								<br />
								例:<br />
								会员于10月1日完成第一笔300元存款，即可获取一次游戏机会。会员随后再存款2,700元，当日总存款累积至3,000元，便可进行剩余的游戏次数。<br />
								<br />
								<br />
								- 会员点击绽放的“烟花”以激活奖品。​<br />
								- 未进行的游戏次数不会累计至次日。​<br />
								...
								3. 此活动将以免费彩金与国庆大礼包为奖品。<br />
								<br />
								4. 派彩时间：<br />
								免费彩金（主钱包）：得奖后30分钟内。<br />
								{/* 乐币（天王俱乐部）：得奖后30分钟内。<br /> */}
								国庆大礼包（礼品）：确认收货信息之后的30天内。<br />
								{/* <br />
							5. 乐币：<br />
