...
export default class SportsBtiDemo extends React.PureComponent {
	...
	componentDidMount() {
		console.log('BTI DEMO DidMount')
	}

	componentWillUnmount(){
		console.log('BTI DEMO WillUnmount')
		// 在componentWillUnmount時 刪掉輪詢
		VendorBTI.deletePolling(this.SportsPollingKey);
		VendorBTI.deletePolling(this.EventsPollingKey);
		VendorBTI.deletePolling(this.OneEventPollingKey);
	}

	//輪詢 體育計數
	SportsPolling() {
		//輪詢 體育計數
		this.SportsPollingKey = VendorBTI.getSportsPolling(PollingResult => {
	...
	EventsPolling() {
		...
		this.EventsPollingKey =VendorBTI.getEventsPolling(PollingResult => {
	...
	//輪詢 單個比賽數據(在比賽詳情頁 使用) 返回輪詢key，在componentWillUnmount時記得刪掉輪詢
	OneEventPolling() {
		this.OneEventPollingKey = VendorBTI.getEventDetailPolling(PollingResult => {
	...
	Search() {
		VendorBTI.search('英').then(datas => {
			...
			if (datas.length <= 0) {
				...
				VendorBTI.getEventsDetail(eventInfos).then(eDatas => {
	...
	SingleBet_demo() {
		...
		VendorBTI.getEvents(BTISports.SOCCER,VendorMarkets.RUNNING,SortWays.EventTime).then(PollingResult =>{
			//獲取比賽數據，這裡用的是足球 早盤
			...
			const pollingKey = VendorBTI.getBetInfoPolling(onUpdateHandler, WagerType.SINGLE, selections[0]); //WagerType.SINGLE表示單注
			...
			const userClickBet = ()=> {
				if (this.state.BetInfoData) {
					...
					if (!this.state.BetInfoData.BetSettings) {
						...
						VendorBTI.deletePolling(pollingKey);  //這裡因為是整個投注檢查+投注一起跑的，才刪除輪詢，實際應用是分開觸發的，不需要刪除
						return;
					}
					...
					VendorBTI.placeBet(WagerType.SINGLE, this.state.BetInfoData, betAmount)
						.then(betResult => {
							...
							//下注成功才 關閉輪詢
							VendorBTI.deletePolling(pollingKey);
	...
	SingleBet_outright_demo() {
		...
		//例子：直接拿 第一個比賽的 第一個玩法 第一個投注選項
		//實際投注頁 不需要這個查詢步驟  直接用 鼠標點擊選到的 selectionData 數據即可
		VendorBTI.getEvents(BTISports.SOCCER, VendorMarkets.OUTRIGHT, SortWays.LeagueName).then(PollingResult =>{
			//獲取比賽數據，這裡用的是足球 優勝冠軍
			const eventDatas = PollingResult.NewData;
			...
			setTimeout(() => {VendorBTI.deletePolling(pollingKey)},20*1000);  //用setTimout 10秒 等輪詢拿到數據，實際投注介面不需要這個，從UI面去限制用戶點擊動作
	...
	ComoboBet_demo() {
		...
		// 實際投注頁 不需要這個查詢步驟  直接用 鼠標點擊選到的 selectionData 數據即可
		VendorBTI.getEvents(BTISports.SOCCER, VendorMarkets.TODAY, SortWays.EventTime).then(PollingResult =>{
			//獲取比賽數據，這裡用的是足球 早盤
			const eventDatas = PollingResult.NewData;
			...
			//輪詢 (檢查)投注選項並獲取投注上下限 等信息
			const pollingKey = VendorBTI.getBetInfoPolling(onUpdateHandler, WagerType.COMBO, selections);  //WagerType.COMBO表示串關
			...
			const userClickBet = ()=> {
				if (this.state.BetInfoData) {
					...
					VendorBTI.deletePolling(pollingKey);
					//執行下注，返回是BetResultData格式
					VendorBTI.placeBet(WagerType.COMBO, this.state.BetInfoData, betAmount, comboType)
						.then(betResult => {
							console.log('placeBet result', betResult)
	...
	notification_demo() {
		...
		const getEventInfosFunc = async () => {
			// 1. 先取用戶配置
			const memberSetting = VendorBTI.getMemberSetting();
			...
			if (memberSetting.goalNotification) {
				...
				if (memberSetting.goalAllRB) {
					const runningPR = await VendorBTI.getEvents(targetSportId,VendorMarkets.RUNNING);
					...
					if (memberSetting.goalMyFavorite) {
						const favEvents = VendorBTI.getFavouriteEvents();
					...
					if (memberSetting.goalIBet) {
						...
						const unsettleWagers = await VendorBTI.getUnsettleWagers();
		...
		this.eventPollingKey = VendorBTI.getEventsDetailPolling(PollingResult => {
			console.log('update Event',PollingResult);
			this.setState({EventDetail:PollingResult.NewData});
	...
	//查詢未結算注單
	getUnsettleWagers() {
		VendorBTI.getUnsettleWagers()
			.then(data => console.log('getUnsettleWagers',data));
	}

	//查詢已結算注單
	getSettledWagers() {
		VendorBTI.getSettledWagers('2020-09-01','2020-12-30')
			.then(data => console.log('getSettledWagers',data));
	}

	//獲取公告
	getAnnouncements() {
		VendorBTI.getAnnouncements()
	...
	Favourite_demo(){
		//添加收藏
		console.log('beofore add event', VendorBTI.getFavouriteEvents());  //獲取收藏數據
		VendorBTI.addFavouriteEvent(13770797, BTISports.SOCCER,false);
		console.log('add event', VendorBTI.getFavouriteEvents());
		VendorBTI.addFavouriteEvent(14025099, BTISports.SOCCER,false);
		console.log('add event', VendorBTI.getFavouriteEvents());
		//刪除收藏
		VendorBTI.removeFavouriteEvent(13203772);
		console.log('remove event', VendorBTI.getFavouriteEvents());
	}

	//獲取Banner數據
	async getBannerData() {
		VendorBTI.getBannerData(BTISports.SOCCER).then(eventDatas => {
	...
	async getLiveEventsInSameLeague() {
		...
		VendorBTI.getLiveEventsInSameLeague(BTISports.SOCCER,"9746","20257767").then(eventDatas => {
	...
	//獲取 足球 早盤 按聯賽排序
	getEvents1() {
		VendorBTI.getEvents(BTISports.SOCCER, VendorMarkets.EARLY, SortWays.LeagueName)
			.then(PollingResult => {
				console.log(PollingResult);
				this.setState({EventDatas:PollingResult.NewData});
			});
	...
	getEvents2() {
		const theDayAfterTwoDays = moment().add(2,'days').format('YYYY-MM-DD');
		VendorBTI.getEvents(BTISports.SOCCER, VendorMarkets.EARLY, SortWays.EventTime,theDayAfterTwoDays,theDayAfterTwoDays)
			.then(PollingResult => {
				console.log(PollingResult);
				this.setState({EventDatas:PollingResult.NewData});
			});
	...
	//獲取 足球 滾球 按時間排序
	getEvents3() {
		VendorBTI.getEvents(BTISports.SOCCER,VendorMarkets.RUNNING, SortWays.EventTime)
			.then(PollingResult => {
				console.log(PollingResult);
				this.setState({EventDatas:PollingResult.NewData});
			});
	...
	//獲取 足球 滾球 按聯賽排序
	getEvents4() {
		VendorBTI.getEvents(BTISports.SOCCER,VendorMarkets.OUTRIGHT, SortWays.LeagueName)
			.then(PollingResult => {
				console.log(PollingResult);
				this.setState({EventDatas:PollingResult.NewData});
			});
	...
	//獲取單個比賽數據
	getEventDetail() {
		VendorBTI.getEventDetail(BTISports.SOCCER, 12656946)
			.then(PollingResult => {
				console.log(PollingResult);
				this.setState({EventDetail:PollingResult.NewData});
			});
	...
	getEventsDetail() {
		...
		VendorBTI.getEventsDetail(events)
			.then(PollingResult => {
	...
	//查看數據用
	async testData() {
		const earlys = await VendorBTI.getEvents(BTISports.SOCCER, VendorMarkets.EARLY, SortWays.EventTime);
		const todays = await VendorBTI.getEvents(BTISports.SOCCER, VendorMarkets.TODAY, SortWays.EventTime);
		const lives = await VendorBTI.getEvents(BTISports.SOCCER, VendorMarkets.RUNNING, SortWays.EventTime);
		...
		VendorBTI.getEventDetail(BTISports.SOCCER, 13890324)
			.then(PollingResult => {
				console.log(PollingResult);
				this.setState({EventDetail:PollingResult.NewData});
			});

		VendorBTI.getEventDetail(BTISports.SOCCER, 13910839)
			.then(PollingResult => {
				console.log(PollingResult);
				this.setState({EventDetail:PollingResult.NewData});
			});
	...
	async tesSingleSelectionDataError() {
		...
		const betInfoPolling = await VendorBTI.getBetInfo(WagerType.SINGLE,selections[0]);
	...
	async testComboSelectionDataError() {
		...
		//仍然會正常返回，但是BetSetting為空，而且不能用的Selection會標記出來(SelectionStatusName)
		const betInfoPolling = await VendorBTI.getBetInfo(WagerType.COMBO,selections);
	...
	async testIsOpenParlay() {
		...
		const betInfoPolling = await VendorBTI.getBetInfo(WagerType.COMBO, selections); //這裡沒事，可以正常過

		console.log('betInfoPolling', betInfoPolling);

		//默認會在placeBet才爆出錯誤，直接回一個 BetStatusMessage = 1126，並且不會告訴你哪個selection有問題
		//但這樣足夠了，在UI點擊的時候事先擋住，遇到這種問題的可能性就很低
		// VendorBTI.placeBet(WagerType.COMBO, betInfoPolling.NewData, 6, betInfoPolling.NewData.BetSettings[0].ComboType)
	...
	async testOneEventOneSelection() {
		...
		const betInfoPolling = await VendorBTI.getBetInfo(WagerType.COMBO,selections);  //會throw 只能選一個Selection 這樣比較明確，而且如果UI點擊的時候事先擋住 就不會有問題
	...
	async testParlaySelectionCountMax() {
		...
		//選幾個投注選項
		let selectionCount = VendorBTI.configs.MaxParlay + 1;
		...
		const betInfoPolling = await VendorBTI.getBetInfo(WagerType.COMBO,selections);  //會throw錯誤
	...
	async tesSingleBetError() {
		...
		//執行下注，返回是BetResultData格式
		VendorBTI.placeBet(WagerType.SINGLE, betInfo, betAmount)
			.then(betResult => console.log('placeBet result', betResult))
			.catch(error => {
				//這裡處理下注可能返回的錯誤
				if (typeof error  === 'object' && error.isVendorError === true) {
	...
	async tesSingleBetOddsChange() {
		...
		if (this.state.tPollingKey) {
			VendorBTI.deletePolling(this.state.tPollingKey);
		...
		//選一個滾球比賽
		const pollingResult = await VendorBTI.getEventDetail(BTISports.SOCCER, eventId);
		...
		//輪詢 (檢查)投注選項並獲取投注上下限 等信息
		const pollingKey = VendorBTI.getBetInfoPolling(onUpdateHandler, WagerType.SINGLE, selection); //WagerType.SINGLE表示單注
	...
	async tesSingleBetWithChangeOdds() {
		...
		//執行下注，返回是BetResultData格式
		VendorBTI.placeBet(WagerType.SINGLE, betInfo, betAmount, 'Single', acceptAny)
			.then(betResult => {
				console.log('placeBet result', betResult)
				//投注成功才 刪除輪詢
				if (this.state.tPollingKey) {
	...
	async testFavEvents(){
		//拿取今日前3個比賽
		const todayEvents = await VendorBTI.getEvents(BTISports.SOCCER,VendorMarkets.TODAY);
		...
		const dumpFavs = async (eventIds) => {
			...
			const events = await VendorBTI.getEventsDetail(eventIds.map(ev => new EventInfo(ev,BTISports.SOCCER,false)));
			events.NewData.map(i => console.log('getEventsDetail', i.EventId, i.IsFavourite));
			await Promise.all(eventIds.map(async eid => {
				const thise = await VendorBTI.getEventDetail(BTISports.SOCCER, eid);
		...
		//添加關注
		console.log('before add', VendorBTI.getFavouriteEvents());
		eventIds.map(i => VendorBTI.addFavouriteEvent(i, BTISports.SOCCER,false));
		console.log('after add', VendorBTI.getFavouriteEvents());

		await dumpFavs(eventIds); //這裡應該全true

		//刪除關注
		console.log('before remove', VendorBTI.getFavouriteEvents());
		eventIds.map(i => VendorBTI.removeFavouriteEvent(i));
		console.log('after remove', VendorBTI.getFavouriteEvents());
	...
	async testbticombobonus() {
		...
		while(selections.length < selectionCount && cc < eventDatas.length) {
			...
			const thisEventDetailPR = await VendorBTI.getEventDetail(BTISports.SOCCER,thisEvent.EventId);
		...
		const betInfoPR = await VendorBTI.getBetInfo(WagerType.COMBO,selections);
		...
		//執行下注，返回是BetResultData格式
		VendorBTI.placeBet(WagerType.COMBO, betInfo, betAmount, targetBetSetting.ComboType)
			.then(betResult => {
				console.log('placeBet result', betResult)
			})
			.catch(error => {
	...
	async testbtifreebet() {
		...
		const betInfoPR = await VendorBTI.getBetInfo(WagerType.SINGLE,selections);
		const betInfo = betInfoPR.NewData;
		console.log('betInfo',betInfo);
		...
		//執行下注，返回是BetResultData格式
		VendorBTI.placeBet(WagerType.SINGLE, betInfo, betAmount, 'Single',false ,targetFreebet.FreeBetToken)
			.then(betResult => {
				console.log('placeBet result', betResult)
			})
			.catch(error => {
	...
	dologin() {
		this.mockLoginPromise = MockLogin.doMockLoginFUN88().then(()=>MockLogin.getBTIToken(true,'fun88').then(() => VendorBTI.BTILogin(MockLogin.BTI_Token)));
	}
