...
const prizeNameMap = { P1: '免费彩金', P3: '乐币', P4: '五一惊喜', P5: '没有奖励' };
...
export default class ann14 extends Component {
	constructor(props) {
		...
		this.state = {
			...
			option: {
				...
				time: 4000, //非必要默认4000
				size: 8, //必须参数，分几个奖项
				bg: `${process.env.BASE_PATH}/img/events/LaborDay2023/M1/LuckyWheel/M1_Lucky-Wheel-Frame.png`, //必要参数，轉盤背景
				btn: `${process.env.BASE_PATH}/img/events/LaborDay2023/M1/LuckyWheel/M1-Button-Static.gif`, //必要参数
				btning: `${process.env.BASE_PATH}/img/events/LaborDay2023/M1/LuckyWheel/M1-Button-Clicked.gif`, //必要参数
				disabledbtn: `${process.env.BASE_PATH}/img/events/LaborDay2023/M1/LuckyWheel/M1-Button-Disabled.png` //必要参数
			}
		};
	...
	getGameInfo = async () => {
		...
		if (targetData) {
			...
			console.log("gameInfo", targetData)
	...
	needLogin = () => {
		if (!checkIsLogin()) {
			redirectToLogin('?from=event_LaborDay2023');
			return true;
		}
		return false;
	};
	...
	showMyPrizes = async () => {
		...
		// Pushgtagdata('Engagement_Event', 'View', 'MyPrize_LaborDay2023​');
	...
	afterOpenPrizePopup = () => {
		...
		//增加 ?日期數字 用來強制加載動畫
		const imgUrl =
			`${process.env.BASE_PATH}` + '/img/events/LaborDay2023/Prize-Popup-BG.gif' + '?' + new Date().getTime();
	...
	drawPrize = async (clickBubble = true) => {
		...
		if (phase != 2) return;
		if (clickBubble) {
			// Pushgtagdata('Engagement_Event', 'Claim', 'SpinLuckyWheel_LaborDay2023​');
		} else {
			// Pushgtagdata('Engagement_Event', 'Click', 'SpinMore_LaborDay2023​');
		}
	...
	clickDeposit = () => {
		// Pushgtagdata('Deposit_Nav', 'Click', 'Deposit_LaborDay2023​');
		Router.push('/deposit');
	};
	...
	render() {
		...
		return (
			...
			>
				<div className="ann14-container labor-day-2023">
					<div className="ann14-top">
						<ReactIMG className="ann14-top-banner" src="/img/events/LaborDay2023/M1/M1_Title.png" />
						<div className="ann14-top-text-container">
							<div className="ann14-top-text">4月29日至5月3日 劳模起来嗨</div>
						</div>
								...
								<div className="ann14-top-button-deposit">
									...
									<div className="ann14-top-button-deposit-row">
										...
										<ReactIMG
											...
											src="/img/events/LaborDay2023/refresh-icon.svg"
											onClick={() => {
						...
						>
							...
							<div className="ann14-top-popup-content">
								<div className="ann14-top-popup-desc">
									{hintType == 1 ? (
										<React.Fragment>
											存款 300 元即可获取 1 次参与活动的机会！ <br />存得越多，机会越多
										</React.Fragment>
									) : null}
									{hintType == 2 ? (
										<React.Fragment>
											您今日的存款不足 300 元， <br />请先存款后再次参与活动。
										</React.Fragment>
									) : null}
									{hintType == 3 ? (
										<div className="ann14-top-popup-desc-single">您今天的活动次数已用完，请明天再试。</div>
									) : null}
									{hintType == 4 ? (
										<div className="ann14-top-popup-desc-single">您的活动次数已用完，请存款后再试。</div>
									) : null}
									{hintType == 5 ? (
										<React.Fragment>
											很遗憾您的转盘是空的，请再接再厉！ <br />今天剩余 {remainingGrabFromCurrentTier} 次参与活动的机会
										</React.Fragment>
									) : null}
									{hintType == 6 ? (
										<React.Fragment>
											很遗憾您的转盘是空的，请再接再厉！ <br />您参与活动的次数已用完，请存款后再试。
										</React.Fragment>
									) : null}
									{hintType == 7 ? (
										<React.Fragment>
											很遗憾您的转盘是空的，请再接再厉！ <br />您今日参与活动的次数已用完，请明天再试
										</React.Fragment>
									) : null}
									{hintType == 8 ? (
									...
									{hintType == 10 ? <React.Fragment>抱歉，该活动已结束，请期待我们的下一个活动</React.Fragment> : null}
							...
							{this.state.showPrizeResult ? (
								<div className="ann14-prize-popup-content">
									...
									<ReactIMG
										...
										src={'/img/events/LaborDay2023/M1/Prizes/' + this.state.prizeType + '.png'}
									/>
									...
									{prizeResultType == 2 ? (
										<div className="ann14-prize-popup-content-text">您的活动次数已用完，请存款后再试。</div>
									) : null}
									{prizeResultType == 3 ? (
										<div className="ann14-prize-popup-content-text">您今天的活动次数已用完，请明天再试。</div>
									) : null}
									<div className="ann14-prize-popup-buttons">
										{prizeResultType == 1 ? (
											<React.Fragment>
												<div
													className="ann14-prize-popup-button-return"
												...
												<div
													className="ann14-prize-popup-button-continue"
													onClick={() => {
														//先關閉彈窗，然後抽獎
					...
					<div className="ann14-middle">
						...
						<div className="ann14-middle-content">
							...
							<div className="ann14-middle-steps-container">
								<div className="ann14-middle-step ann14-middle-step1">
									<ReactIMG
										...
										src="/img/events/LaborDay2023/How-To-Step-1.png"
									/>
								...
								<div className="ann14-middle-step ann14-middle-step2">
									<ReactIMG
										className="ann14-middle-step-img"
										src="/img/events/LaborDay2023/How-To-Step-2.png"
									/>
								...
								<div className="ann14-middle-step ann14-middle-step3">
									<ReactIMG
										className="ann14-middle-step-img"
										src="/img/events/LaborDay2023/How-To-Step-3.png"
									/>
									<div className="ann14-middle-step-title-1">步骤三</div>
									<div className="ann14-middle-step-title-2">即刻抽奖赢好礼</div>
								</div>
							</div>
							<div className="ann14-middle-button-container">
								<div
									...
									onClick={() => {
										// Pushgtagdata('Engagement_Event', 'View', 'TnC_LaborDay2023​');
						...
						<ReactModal
							...
							overlayClassName="ann14-popup-overlay"
              style={{content:{width:"88%",height:"520px", paddingBottom:"15px", overflowX:"hidden"}}}
						>
							<div className="ann14-middle-popup-header">
								<div>规则与条款</div>
								<ReactIMG
									className="ann14-middle-popup-close"
									src="/img/events/LaborDay2023/Icon-Close.png"
									onClick={() => this.setState({ showTNC: false })}
								/>
							</div>
							<div className="ann14-middle-popup-content">
								1. 此活动开放给所有乐天堂会员<br />
								活动时间：4月29日 00:00 至 5月3日 23:59 (北京时间）<br />
								<br />
								2. 参加方式:<br />
								- 游戏次数将以会员当天累计存款总额为标准，最低<br/>存款为300元，如下图：<br />
								<br />
								...
								<br />
								例:<br />
								会员于4月29日完成第一笔300元存款到账后，即可获取<br/>一次游戏机会。会员其后再存款2,700元，当日总存款到<br/>帐金额累积至3,000元，便可进行剩余的游戏次数。<br />
								<br />
								<br />
								- 当日游戏次数最多为5次。 ​<br />
								- 会员点击“旋转”以激活奖品。​<br />
								...
								- 游戏次数以存款到账金额计算。​<br />
								<br />
								3. 此活动将以免费彩金与五一惊喜礼包为奖品。<br />
								<br />
								4. 派彩时间：<br />
								免费彩金（主钱包）：得奖后30分钟内。<br />
								五一惊喜（礼品）：确认收货信息之后的30天内。<br />
								<br />
								5. 免费彩金：<br />
								彩金自动记入会员主钱包后，有效期为30天。1倍流水方<br/>可提款。<br />
								<br />
								6. 五一惊喜（礼品）：<br />
								- 礼品将随机派发，该礼品将不会透露直到会员收件。<br />
								- 天使客服将在活动结束后7天之内联系中奖会员，索取<br/>收件信息。<br />
								- 若礼品已出库，因收件信息不完整，物流公司无法联系<br/>会员而被退回，乐天堂有权撤销该礼品。<br />
              ...
              {/* 待提供api撈取LaborDay2023 Prize list data */}
							{prizeList.map((pn) => {
                console.log("prizeList",pn)
								return (
									<ReactIMG
										key={pn}
										className="ann14-bottom-prize-img"
										src={'/img/events/LaborDay2023/M1/Prizes/' + pn + '.png'}
									/>
								);
							})}
						</div>
						...
						<ReactModal
							...
							className="MyPrizes ann14-bottom-popup labor-day-2023"
							overlayClassName="ann14-popup-overlay"
							style={{content:{paddingBottom:"20px"}}}
						>
							...
							<div className="ann14-bottom-popup-content" style={{minHeight:"120px", maxHeight:"350px"}}>
								<table className="ann14-bottom-popup-table">
									<tbody>
										<tr>
											...
											{/* For dummy test***
											{Array.from({length:20},cur=> cur=<tr style={{fontSize:"11px"}}>
												<td>2022-09-15 18:00</td>
												<td>88元免费彩金</td>
												<td>123456</td>
											</tr>)} */}
										{myPrizes && myPrizes.length > 0 ? (
											...
											<tr>
												<td colSpan="3" style={{color:"#596481"}}>没有获奖记录</td>
											</tr>
										)}
									</tbody>
								</table>
