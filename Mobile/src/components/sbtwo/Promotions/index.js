...
class _Promotions extends React.Component {
			...
			if (categories) {
				...
				PromotionList(params).then((res) => {
					let filterData = res.filter((element) => {
						return element.category.includes('Sports');
					});
					this.setState({
						DefaultPromotion: filterData,
    ...
    */
	...
	render() {
		...
		return (
			<React.Fragment>
				<div id="promotions" className="SB20Proms" style={{ paddingTop: '10px' }}>
								...
								return (
									...
									>
										...
										<div className="promoTime">
											...
											{moment(item.startDate).utcOffset(8).format('YYYY-MM-DD HH:mm:ss')}-
											{moment(item.endDate).utcOffset(8).format('YYYY-MM-DD HH:mm:ss')}
										</div>
									</div>
								);
							})}
