...
import { GetWalletBonus, GetTransferWalletBonus } from '$SBTWOLIB/data/wallet';
	...
	getDepositWalletBonus(type) {
		...
		if (this.props.type == 'deposit') {
			...
			GetWalletBonus(type, (res) => {
				...
				typeof this.setWalletBonus === 'function' && this.setWalletBonus(type, res);
			});
		} else {
			/* --------------转账红利列表---------- */
		...
		fetchRequest(ApiPort.PromoCalculate, 'POST', data)
			.then((data) => {
				...
				if (data.previewMessage != '') {
					...
					this.setState({
						previewMessage: data.errorMessage
					});
				}
			})
			.catch(() => {
								...
								{bonusList.map((value) => {
									return (
										<Option value={value.id} key={value.id}>
											{value.title === 'fun88_ApplyNextTime' ? '下回再参与！' : value.title}
										</Option>
									);
								})}
							</Select>
