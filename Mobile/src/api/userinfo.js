        ...
        fetchRequest(ApiPort.Member, "GET")
            .then((res) => {
                if (res && res.isSuccess && res.result) {
                    Object.assign(memberInfos, res.result.memberInfo);
                    Object.assign(memberInfos, res.result.memberNewInfo);
                }
                // const memberInfoString = JSON.stringify(memberInfos);
...
//验证充值之前 填写资料的次数 如果失败 就直接进入 超过验证次数限制页面
export function getResendAttempt(
    call,
    serviceAction = "BankCardVerification",
    channel = "SMS"
) {
    fetchRequest(
        `${ApiPort.ResendAttempt}serviceAction=${serviceAction}&channelType=${channel}&`,
        "GET"
    )
        .then((res) => {
            call(res);
        ...
        .catch((error) => {
            console.log("getResendAttempt error:", error);
            return null;
        });
}

//存款前 验证真实姓名和身份证
export function setUserRealyNameAndIdCard(
    name,
    id,
    bankName,
    accountNumber,
    ...
    fetchRequest(ApiPort.PostBankCardVerification, "POST", {
        realName: name,
        bankName: bankName,
        ...
        .catch((error) => {
            console.log("setUserRealyNameAndIdCard error:", error);
            return null;
        });
}

/**
 * @description: 提交身份证资料 提款
 * @param {*} userInfo
 * @param {*} call
 * @return {*}
 */
export function setUserInfo(userInfo, call) {
    fetchRequest(ApiPort.Member, "PATCH", userInfo)
        .then((res) => {
            if (res) {
                // if (res.isSuccess == true) {
                // 	Toast.success('更新成功!');
 ...
 * @return {*}
 */
export function setUserAddressInfo(userInfo, call) {
    fetchRequest(ApiPort.Member, "PUT", userInfo)
        .then((res) => {
            if (res) {
                // if (res.isSuccess == true) {
                // 	Toast.success('更新成功!');
        ...
        .catch((error) => {
            console.log("setUserAddressInfo error:", error);
            return null;
        });
}

export function setMemberInfo(data, call) {
    fetchRequest(ApiPort.PUTMemberlistAPI, "PATCH", data)
        .then((res) => {
            call(res);
        })
        .catch((error) => {
...
export function setMemberInfoPUT(data, call) {
    fetchRequest(ApiPort.PUTMemberlistAPI, "PUT", data)
        .then((res) => {
            call(res);
        })
        .catch((error) => {
...
export function getQuestion(call) {
    fetchRequest(ApiPort.GetQuestions, "GET")
        .then((res) => {
            call(res);
        })
        .catch((error) => {
...
export function backToMainsite(link) {
    ...
    if (StagingApi) {
        ...
        if (url && token) {
            let mainsiteServiceLink = `${url.protocol}//${
                url.hostname
            }/Services/SBV2Service.ashx?token=${
                token.split(" ")[1]
            }&rtoken=${refreshToken}&cultureLanguage=zh-cn`;
        ...
        targetDomain =
            url.hostname.split(".").length == 3
                ? `${url.protocol}//${url.hostname.split(".")[1]}.${
                      url.hostname.split(".")[2]
                  }`
        ...
        if (url && token) {
            let mainsiteServiceLink = `${targetDomain}/Services/SBV2Service.ashx?token=${
                token.split(" ")[1]
            }&rtoken=${refreshToken}&cultureLanguage=zh-cn`;
            console.log(mainsiteServiceLink);
            window.location.href = mainsiteServiceLink;
...
// 更新密码
export function UpdatePassWord(data, call) {
    fetchRequest(ApiPort.ChangePassword, "PUT", data)
        .then((res) => {
            call(res);
        })
        .catch((error) => {
...
/**
 * @description: 重置密码，验证邮箱token是否可行
 * @param {*} data
 * @param {*} call
 * @return {*}
 */
export function VerificationEmailToken(params, call) {
    fetchRequest(
        ApiPort.VerificationEmailToken + Qs.stringify(params) + "&",
        "PATCH"
    )
        .then((res) => {
            call(res);
        ...
        .catch((error) => {
            console.log("VerificationEmailToken error:", error);
            return null;
        });
}
 ...
 * @return {*}
 */
export function ResetPassword(data, call) {
    fetchRequest(ApiPort.ResetPasswordLink, "PUT", data)
        .then((res) => {
            call(res);
        })
        .catch((error) => {
